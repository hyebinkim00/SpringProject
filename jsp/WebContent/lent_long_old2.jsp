...
String selname1 =request.getParameter("sellist1");
%>
...
<hr style="border: solid 1px black " /> 
	...
	<div class="row" style="border: 1px solid #eeeeee">
			...
			<select id="sellist1" name="sellist1" onchange="goDate()">
			...
			select * from member where facname=?
			 <sql:param value="<%=selname1 %>"/>
			</sql:query>
				<select id="sel" onchange="tee()">
				<c:forEach var="row" items="${resultSet.rows}">
				<option value="<c:out value="${row.carname}"/>"><c:out value="${row.carname}"/></option>
				</c:forEach>
				</select>
				<!-- <p>
